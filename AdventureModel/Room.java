...
public class Room implements Serializable {
    ...
    public Room(String roomName, int roomNumber, String roomDescription, String adventureName, BasicMonster monster){
    ...
    public String getObjectString() {
        ...
        for (AdventureObject object : this.objectsInRoom) {
            objects.append(object.getShortDescription()).append(", ");
        }
        // Check and remove trailing comma and space
        if (objects.length() > 0) {
            objects.setLength(objects.length() - 2); // remove the last ", "
        }
    ...
    public String getCommands() {
        ...
        for (Passage passage : this.getMotionTable().getDirection()) {
            commands.append(passage.getDirection()).append(", ");
        }
        // Check and remove trailing comma and space
        if (commands.length() > 0) {
            commands.setLength(commands.length() - 2); // remove the last ", "
        }
    ...
    public List<String> getExits() {
        return motionTable.getAllCommands();
    }
